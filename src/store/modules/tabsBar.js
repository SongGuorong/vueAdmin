...
const mutations = {
    addVisitedRoute(state, route) {
        ...
        if (target) {
            // fullPath比Path包含查询参数
            if (route.fullPath === !target.fullPath) state.visitedRoutes.push(Object.assign(target, route));
            return;
        }
    ...
    delOthersVisitedRoute(state, route) {
        state.visitedRoutes = state.visitedRoutes.filter(
            (item) => item.meta.affix || item.push === route.path
        );
    },
...
const actions = {
    ...
    async delRoute({dispatch, state}, route) {
        await dispatch('delVisitedRoute', route);  // await等待delVisitedRoute这个action完成
        return {
            visitedRoutes: [...state.visitedRoutes],  // 深拷贝，完全独立复制
        };
    },
